...
module.exports = HandleMsg = async (m2, message) => {
    try {
        ...
        const botNumber = await m2.getHostNumber() + '@c.us'
        const groupId = isGroupMsg ? chat.groupMetadata.id : ''
        const groupAdmins = isGroupMsg ? await m2.getGroupAdmins(groupId) : ''
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            .then(() => ((isGroupMsg) && (isGroupAdmins)) ? m2.sendText(from, `Menu Admin Grup: *${prefix}menuadmin*`) : null)
            break
        case 'menuadmin':
            ...
            await m2.sendText(from, menuId.textAdmin())
            break
        case 'donate':
        case 'donasi':
        ...
        case 'ownerbot':
            ...
            .then(() => m2.sendText(from, 'Jika kalian ingin request fitur silahkan hubungi owner!'))
            break
        case 'join':
            if (args.length == 0) return m2.reply(from, `Jika kalian ingin mengundang bot kegroup silahkan invite atau dengan\nketik ${prefix}join [link group]`, id)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await m2.inviteInfo(linkgrup)
            if (!islink) return m2.reply(from, 'Maaf link group-nya salah! silahkan kirim link yang benar', id)
            if (isOwnerBot) {
                await m2.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await m2.sendText(from, 'Berhasil join grup via link!')
                          await m2.sendText(chekgrup.id, `Hi buddy~, I m M2 BOT but you can call me M. To find out the commands for this bot services type ${prefix}menu`)
                      })
            } else {
                let cgrup = await m2.getAllGroups()
                if (cgrup.length > groupLimit) return m2.reply(from, `Sorry, the group on this bot is full\nMax Group is: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return m2.reply(from, `Sorry, BOT wil not join if the group members do not exceed ${memberLimit} people`, id)
                await m2.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await m2.reply(from, 'Berhasil join grup via link!', id)
                      })
                      .catch(() => {
                          m2.reply(from, 'Gagal!', id)
                      })
            }
            break
        case 'botstat': {
            const loadedMsg = await m2.getAmountOfLoadedMessages()
            const chatIds = await m2.getAllChatIds()
            const groups = await m2.getAllGroups()
            m2.sendText(from, `Status :\n- *${loadedMsg}* Loaded Messages\n- *${groups.length}* Group Chats\n- *${chatIds.length - groups.length}* Personal Chats\n- *${chatIds.length}* Total Chats`)
            break
        }
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    m2.reply(from, 'Berhasil membuat sticker!')
                ...
                m2.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : m2.reply(from, 'Berhasil membuat sticker!')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'giphy':
            ...
            if (isGiphy) {
                ...
                if (!getGiphyCode) { return m2.reply(from, 'Gagal mengambil kode giphy', id) }
                const giphyCode = getGiphyCode[0].replace(/[-\/]/gi, '')
                const smallGifUrl = 'https://media.giphy.com/media/' + giphyCode + '/giphy-downsized.gif'
                m2.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    m2.reply(from, 'Here\'s your sticker')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
                }).catch((err) => console.log(err))
            } else if (isMediaGiphy) {
                const gifUrl = url.match(new RegExp(/(giphy|source).(gif|mp4)/, 'gi'))
                if (!gifUrl) { return m2.reply(from, 'Gagal mengambil kode giphy', id) }
        ...
        case 'nulis':
            ...
            await m2.sendImage(from, `${nulisp}`, '', 'Nih tulisannya!', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'listsurah':
            ...
            break
        case 'infosurah':
            if (args.length == 0) return m2.reply(from, `*_${prefix}infosurah <nama surah>_*\nMenampilkan informasi lengkap mengenai surah tertentu. Contoh penggunan: ${prefix}infosurah al-baqarah`, message.id)
                ...
                m2.reply(from, pesan, message.id)
              break
        case 'surah':
            if (args.length == 0) return m2.reply(from, `*_${prefix}surah <nama surah> <ayat>_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahannya dalam bahasa Indonesia. Contoh penggunaan : ${prefix}surah al-baqarah 1\n\n*_${prefix}surah <nama surah> <ayat> en/id_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahannya dalam bahasa Inggris / Indonesia. Contoh penggunaan : ${prefix}surah al-baqarah 1 id`, message.id)
                var responseh = await axios.get('https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/islam/surah.json')
                var { data } = responseh.data
                var idx = data.findIndex(function(post, index) {
                  if((post.name.transliteration.id.toLowerCase() == args[0].toLowerCase())||(post.name.transliteration.en.toLowerCase() == args[0].toLowerCase()))
              ...
              break
        case 'tafsir':
            if (args.length == 0) return m2.reply(from, `*_${prefix}tafsir <nama surah> <ayat>_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahan dan tafsirnya dalam bahasa Indonesia. Contoh penggunaan : ${prefix}tafsir al-baqarah 1`, message.id)
              ...
              bhs = ""
                ...
                if(!isNaN(nmr)) {
                  ...
                  if(isNaN(ayat)) {
                    ...
                    pesan = pesan + "\n\n(Q.S. "+data.surah.name.transliteration.id+":"+args[1]+")"
                    await m2.sendFileFromUrl(from, data.audio.secondary[0])
                    await m2.reply(from, pesan, message.id)
                  }
              }
              break
        case 'jsolat':
            if (args.length == 0) return m2.reply(from, `Untuk melihat jadwal solat dari setiap daerah yang ada\nketik: ${prefix}jsolat [daerah]\n\nuntuk list daerah yang ada\nketik: ${prefix}daerah`, id)
            const solatx = body.slice(8)
            const solatj = await rugaapi.jadwaldaerah(solatx)
            await m2.reply(from, solatj, id)
            .catch(() => {
                m2.reply(from, 'Sudah input daerah yang ada dilist?', id)
            })
            break
        case 'daerah':
            const daerahq = await rugaapi.daerah()
            await m2.reply(from, daerahq, id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        // Social Media
	   case 'twimg':
            if (args.length == 0) return m2.reply(from, `Untuk mendownload gambar dari twitter\nketik: ${prefix}twimg [link_twitter]`, id)
            const twimg = await rugaapi.twitimg(args[0])
            await m2.sendFileFromUrl(from, twimg, 'Sukses mengunduh Foto Twitter Menggunakan Bot WhatsApp M2', '', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
        case 'twvid':
            if (args.length == 0) return m2.reply(from, `Untuk mendownload video dari twitter\nketik: ${prefix}twvid [link_twitter]`, id)
			await m2.reply(from, `_Tunggu, sedang memproses perintah_`, id)
            const twvid = await rugaapi.twitvid(args[0])
            await m2.sendFileFromUrl(from, twvid, 'Sukses mengunduh Video Twitter Menggunakan Bot WhatsApp M2', '', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
        case 'insta':
		case 'ig':
            if (args.length == 0) return m2.reply(from, `Untuk mendownload gambar atau video dari instagram\nketik: ${prefix}insta [link_ig]`, id)
			await m2.reply(from, `_Tunggu, sedang memproses perintah_`, id)
            const instag = await rugaapi.insta(args[0])
            await m2.sendFileFromUrl(from, instag, 'Sukses mengunduh Foto/Video Instagram Menggunakan Bot WhatsApp M2', '', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
		case 'tiktok':
        case 'tt':
            if (args.length == 0) return m2.reply(from, `Untuk mendownload video tiktok\nketik: ${prefix}tiktok [link_tik]`, id)
			await m2.reply(from, `_Tunggu, sedang memproses perintah_`, id)
            const vidTiktok = await rugaapi.tiktok(args[0])
			const infoTiktok = await rugaapi.tiktokInfo(args[0])
			await m2.sendFileFromUrl(from, vidTiktok, '', '', id)
			await m2.sendText(from, infoTiktok, '', '', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
        case 'ytmsc':
            if (args.length == 0) return m2.reply(from, `Untuk mendownload video youtube\nketik: ${prefix}ytmus [link_youtube]`, id)
            await m2.reply(from, `_Tunggu, sedang memproses perintah_`, id)
            const ytmus = await rugaapi.ytmp3(args[0])
            await m2.sendFileFromUrl(from, ytmus, '', '', id)
			await m2.reply(from, `_Sukses Mengunduh Audio Dari Youtube._`, id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
		case 'ytvid':
            ...
            await m2.sendFileFromUrl(from, youtube, `Sukses mengunduh Video Youtube Menggunakan Bot WhatsApp M2`, id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!, mungkin bisa kirim ulang perintah.', id)
            })
            break
        ...
        case 'pantun':
            ...
            .then(body => {
                ...
                m2.reply(from, randompantun.replace(/aruga-line/g,"\n"), id)
            })
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    m2.sendFileFromUrl(from, randomnimex, '', 'Nee..', id)
                })
                .catch(() => {
                    m2.reply(from, 'Ada yang Error!', id)
                })
            } else {
                m2.reply(from, `Maaf query tidak tersedia. Silahkan ketik ${prefix}anime untuk melihat list query`)
            }
            break
        case 'kpop':
            if (args.length == 0) return m2.reply(from, `Untuk menggunakan ${prefix}kpop\nSilahkan ketik: ${prefix}kpop [query]\nContoh: ${prefix}kpop bts\n\nquery yang tersedia:\nblackpink, exo, bts`, id)
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    m2.sendFileFromUrl(from, randomkpopx, '', 'Nee..', id)
                })
                .catch(() => {
                    m2.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'images':
        case 'img':
            if (args.length == 0) return m2.reply(from, `Untuk mencari gambar di pinterest\nketik: ${prefix}images [search]\ncontoh: ${prefix}images naruto`, id)
            const cariwall = body.slice(8)
            const hasilwall = await images.fdci(cariwall)
            await m2.sendFileFromUrl(from, hasilwall, '', '', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
        case 'sreddit':
        case 'reddit':
            if (args.length == 0) return m2.reply(from, `Untuk mencari gambar di sub reddit\nketik: ${prefix}sreddit [search]\ncontoh: ${prefix}sreddit naruto`, id)
            const carireddit = body.slice(9)
            const hasilreddit = await images.sreddit(carireddit)
            await m2.sendFileFromUrl(from, hasilreddit, '', '', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
	    break
        case 'resep':
            if (args.length == 0) return m2.reply(from, `Untuk mencari resep makanan\nCaranya ketik: ${prefix}resep [search]\n\ncontoh: ${prefix}resep tahu`, id)
            const cariresep = body.slice(7)
            const hasilresep = await resep.resep(cariresep)
            await m2.reply(from, hasilresep + '\n\nIni kak resep makanannya..', id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
        /*case 'nekopoi':
            m2.sendText(from, `Maaf fitur ini sedang dalam perbaikan...`)
            rugapoi.getLatest()
            .then((result) => {
		...
		*/
        ...
        case 'cuaca':
            ...
            await m2.reply(from, cuacap, id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
		case 'lirik':
			if (args.length == 0) return m2.reply(from, `Untuk mencari lirik dari sebuah lagu\bketik: ${prefix}chord [judul_lagu]`, id)
			rugaapi.lirik(body.slice(7))
			.then(async (res) => {
				await m2.reply(from, `Lirik Lagu: ${body.slice(7)}\n\n${res}`, id)
			})
			break
        case 'chord':
            if (args.length == 0) return m2.reply(from, `Untuk mencari lirik dan chord dari sebuah lagu\bketik: ${prefix}chord [judul_lagu]`, id)
            const chordq = body.slice(7)
            const chordp = await rugaapi.chord(chordq)
            await m2.reply(from, chordp, id)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'play'://silahkan kalian custom sendiri jika ada yang ingin diubah
            ...
            .then(async (res) => {
                ...
                .then(async(rest) => {
					if (Number(rest.data.filesize.split(' MB')[0]) >= 10.00) return m2.reply(from, 'Maaf ukuran file terlalu besar!')
                    await m2.sendPtt(from, `${rest.data.result}`, id)
                })
                .catch(() => {
                    m2.reply(from, 'Ada yang Error!', id)
                })
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		m2.reply(from, 'Maaf, saya tidak tau anime apa ini, pastikan gambar yang digunakan memiliki kualitas yang baik', id)
                	}
                    const { is_adult, title, title_chinese, title_romaji, title_english, episode, similarity, filename, at, tokenthumb, anilist_id } = resolt.docs[0]
                    teks = ''
                    if (similarity < 0.92) {
                    	teks = '*Apakah ini yang anda maksud* :\n\n'
                    }
                    ...
                    m2.sendFileFromUrl(from, video, 'anime.mp4', teks, id).catch(() => {
                        m2.reply(from, teks, id)
                    })
                })
                .catch(() => {
                    m2.reply(from, 'Ada yang Error!', id)
        ...
        case 'tts':
            ...
            const dataText = body.slice(8)
                ...
                try {
                    ...
                    m2.sendPtt(from, './media/tts.mp3', id)
                    })
                } catch (err) {
                    m2.reply(from, err, id)
                }
        ...
        case 'translate':
            ...
            translate(quoteText, args[0])
                .then((result) => m2.sendText(from, result))
                .catch(() => m2.sendText(from, 'Error, Kode bahasa salah.'))
            break
		case 'covidindo':
			rugaapi.covidindo()
			.then(async (res) => {
			...
			break
        case 'ceklokasi':
            if (quotedMsg.type !== 'location') return m2.reply(from, `Maaf, format pesan salah.\nKirimkan lokasi dan reply dengan caption ${prefix}ceklokasi`, id)
            console.log(`Request Status Zona Penyebaran Covid-19 (${quotedMsg.lat}, ${quotedMsg.lng}).`)
            const zoneStatus = await getLocationData(quotedMsg.lat, quotedMsg.lng)
            if (zoneStatus.kode !== 200) m2.sendText(from, 'Maaf, Terjadi error ketika memeriksa lokasi yang anda kirim.')
            let datax = ''
        ...
        case 'shortlink':
            ...
            await m2.sendText(from, shortlink)
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            break
		...
		case 'klasmen':
            ...
            await m2.sendTextWithMentions(from, textKlas)
			break
	    ...
	    case 'add':
	        ...
	        if (args.length !== 1) return m2.reply(from, `Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
		            ...
		            .then(() => m2.reply(from, 'Hai selamat datang', id))
                } catch {
        ...
        case 'promote':
            ...
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return m2.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length !== 1) return m2.reply(from, 'Maaf, hanya bisa mempromote 1 user', id)
            if (groupAdmins.includes(mentionedJidList[0])) return await m2.reply(from, 'Maaf, user tersebut sudah menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await m2.reply(from, 'Maaf, format pesan salah.\nTidak dapat mempromote akun bot sendiri', id)
            await m2.promoteParticipant(groupId, mentionedJidList[0])
            await m2.sendTextWithMentions(from, `Request diterima, menambahkan @${mentionedJidList[0].replace('@c.us', '')} sebagai admin.`)
            break
        case 'demote':
            if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return m2.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length !== 1) return m2.reply(from, 'Maaf, hanya bisa mendemote 1 user', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await m2.reply(from, 'Maaf, user tersebut belum menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await m2.reply(from, 'Maaf, format pesan salah.\nTidak dapat mendemote akun bot sendiri', id)
            await m2.demoteParticipant(groupId, mentionedJidList[0])
            await m2.sendTextWithMentions(from, `Request diterima, menghapus jabatan @${mentionedJidList[0].replace('@c.us', '')}.`)
            break
        case 'bye':
            if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            m2.sendText(from, 'Good bye... ( ⇀‸↼‶ )').then(() => m2.leaveGroup(groupId))
            break
        case 'del':
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!quotedMsg) return m2.reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
            if (!quotedMsgObj.fromMe) return m2.reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
            m2.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
        case 'tagall':
        case 'everyone':
            if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            const groupMem = await m2.getGroupMembers(groupId)
            ...
            hehex += '╚═〘 *Yaelahdo  B O T* 〙'
            await m2.sendTextWithMentions(from, hehex)
            break
	/*	case 'simisimi':
			if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
			m2.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			break
		case 'simi':
			if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
			if (args.length !== 1) return m2.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			if (args[0] == 'on') {
				simi.push(chatId)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
                m2.reply(from, 'Mengaktifkan bot simi-simi!', id)
			} else if (args[0] == 'off') {
				let inxx = simi.indexOf(chatId)
				simi.splice(inxx, 1)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
				m2.reply(from, 'Menonaktifkan bot simi-simi!', id)
			} else {
				m2.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			}
			break */
		...
		case 'kasar':
			if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
			if (args.length !== 1) return m2.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\napasih itu? fitur apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			if (args[0] == 'on') {
				ngegas.push(chatId)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				m2.reply(from, 'sukses mengaktifkan fitur anti kata kasar', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				m2.reply(from, 'sukses menonatifkan fitur anti kata kasar', id)
			} else {
				m2.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\napasih itu? fitur apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			}
			break
        ...
        if (!isOwner) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai oleh owner grup!', id)
        if (!isBotGroupAdmins) return m2.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            const allMem = await m2.getGroupMembers(groupId)
            for (let i = 0; i < allMem.length; i++) {
                if (groupAdmins.includes(allMem[i].id)) {

                } else {
                    await m2.removeParticipant(groupId, allMem[i].id)
                }
            }
            m2.reply(from, 'Success kick all member', id)
        break

        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return m2.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return m2.reply(from, `Untuk banned seseorang agar tidak bisa menggunakan commands\n\nCaranya ketik: \n${prefix}ban add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
                banned.push(args[1]+'@c.us')
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                m2.reply(from, 'Success banned target!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
                banned.splice(xnxx,1)
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                m2.reply(from, 'Success unbanned target!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                m2.reply(from, 'Success ban target!', id)
                }
            }
            break
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) m2.sendText(idk, `〘 *Yaelahdo  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) m2.sendText(idk, `〘 *Yaelahdo  B C* 〙\n\n${msg}`)
            }
        ...
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            if (!isOwnerBot) return m2.reply(from, 'Perintah ini hanya untuk Owner bot', id)
            const allChatz = await m2.getAllChatIds()
            const allGroupz = await m2.getAllGroups()
            for (let gclist of allGroupz) {
                await m2.sendText(gclist.contact.id, `Maaf bot sedang pembersihan, total chat aktif : ${allChatz.length}`)
                await m2.leaveGroup(gclist.contact.id)
                await m2.deleteChat(gclist.contact.id)
            }
            m2.reply(from, 'Success leave all group!', id)
            break
        case 'clearall': //menghapus seluruh pesan diakun bot
            if (!isOwnerBot) return m2.reply(from, 'Perintah ini hanya untuk Owner bot', id)
            const allChatx = await m2.getAllChats()
            for (let dchat of allChatx) {
                await m2.deleteChat(dchat.id)
            }
            m2.reply(from, 'Success clear all chat!', id)
            break
        default:
            break
        }
		...
		if ((!isCmd && isGroupMsg && isSimi) && message.type === 'chat') {
			...
			.then((res) => {
				if (res.data.status == 403) return m2.sendText(ownerNumber, `${res.data.result}\n\n${res.data.pesan}`)
				m2.reply(from, `Simi berkata: ${res.data.result}`, id)
			})
			.catch((err) => {
				m2.reply(from, `${err}`, id)
			})
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await m2.reply(from, "Jangan badword sayang\nDenda +5.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
