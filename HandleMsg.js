...
module.exports = HandleMsg = async (m2, message) => {
	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await m2.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await m2.sendText(chekgrup.id, `Hi buddy~, I m M2 BOT but you can call me M. To find out the commands for this bot services type ${prefix}menu or ${prefix}help`)
                      })
            } else {
        ...
        case 'gif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    m2.reply(from, `[❗] Kirim gif dengan caption *${prefix}stickergif* atau *${prefix}gif* max 10 sec!`, id)
                   }
                } else {
        ...
        case 'jsolat':
            ...
            .catch(() => {
                m2.reply(from, 'Pastikan daerah kamu ada di list ya!', id)
            })
            break
        case 'daerah':
	...
	case 'grouplink':
            ...
            if (isGroupMsg) {
                ...
                m2.sendLinkWithAutoPreview(from, inviteLink, `\nLink group *${name}* Gunakan *${prefix}revoke* untuk mereset Link group`)
            } else {
            	m2.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            }
            break
	case "revoke":
	if (!isBotGroupAdmins) return m2.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
                    if (isBotGroupAdmins) {
                        m2
                            ...
                            .then((res) => {
                                m2.reply(from, `Berhasil Revoke Grup Link gunakan *${prefix}grouplink* untuk mendapatkan group invite link yang terbaru`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        case 'ytmp3':
            ...
            .then(async(res) => {
				if (res.error) return m2.sendFileFromUrl(from, `${res.url}`, '', `${res.error}`)
				await m2.sendFileFromUrl(from, `${res.result.thumb}`, '', `Lagu ditemukan\n\nJudul: ${res.result.title}\nDesc: ${res.result.desc}\nSabar lagi dikirim`, id)
				await m2.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
				.catch(() => {
		...
		case 'fb':
		case 'facebook':
			if (args.length == 0) return m2.reply(from, `Untuk mendownload video dari link facebook\nketik: ${prefix}fb [link_fb]`, id)
			rugaapi.fb(args[0])
			.then(async (res) => {
				const { link, linkhd, linksd } = res
				if (res.status == 'error') return m2.sendFileFromUrl(from, link, '', 'Maaf url anda tidak dapat ditemukan', id)
				await m2.sendFileFromUrl(from, linkhd, '', 'Nih ngab videonya', id)
				.catch(async () => {
					await m2.sendFileFromUrl(from, linksd, '', 'Nih ngab videonya', id)
					.catch(() => {
						m2.reply(from, 'Maaf url anda tidak dapat ditemukan', id)
					})
				})
			})
			break
        ...
        // Random Kata
      	case 'motivasi':
            ...
            .then(body => {
                ...
                m2.reply(from, randommotivasi, id)
            })
            .catch(() => {
                m2.reply(from, 'Ada yang Error!', id)
            })
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		m2.reply(from, 'Maaf, saya tidak tau ini anime apa, pastikan gambar yang akan di Search tidak Buram/Kepotong', id)
                	}
		...
		case 'kasar':
			...
			if (args[0] == 'on') {
				...
				m2.reply(from, 'Fitur Anti Kasar sudah di Aktifkan', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				m2.reply(from, 'Fitur Anti Kasar sudah di non-Aktifkan', id)
			} else {
				m2.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\nApasih kegunaan Fitur Ini? Apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			}
			break
		...
		case 'mutegrup':
            ...
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return m2.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
			if (args.length !== 1) return m2.reply(from, `Untuk mengubah settingan group chat agar hanya admin saja yang bisa chat\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
            if (args[0] == 'on') {
				m2.setGroupToAdminsOnly(groupId, true).then(() => m2.sendText(from, 'Berhasil mengubah agar hanya admin yang dapat chat!'))
			} else if (args[0] == 'off') {
				m2.setGroupToAdminsOnly(groupId, false).then(() => m2.sendText(from, 'Berhasil mengubah agar semua anggota dapat chat!'))
			} else {
				m2.reply(from, `Untuk mengubah settingan group chat agar hanya admin saja yang bisa chat\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
			}
			break
		case 'setprofile':
			if (!isGroupMsg) return m2.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
			...
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
				await m2.setGroupIcon(groupId, imageBase64)
			} else if (args.length === 1) {
				if (!isUrl(url)) { await m2.reply(from, 'Maaf, link yang kamu kirim tidak valid.', id) }
				m2.setGroupIconByUrl(groupId, url).then((r) => (!r && r !== undefined)
		...
		case 'welcome':
            ...
            if (!isGroupAdmins) return m2.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return m2.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
			if (args.length !== 1) return m2.reply(from, `Membuat BOT menyapa member yang baru join kedalam group chat!\n\nPenggunaan:\n${prefix}welcome on --aktifkan\n${prefix}welcome off --nonaktifkan`, id)
			if (args[0] == 'on') {
				welcome.push(chatId)
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				m2.reply(from, 'Welcome Message sekarang diaktifkan!', id)
			} else if (args[0] == 'off') {
				let xporn = welcome.indexOf(chatId)
				welcome.splice(xporn, 1)
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				m2.reply(from, 'Welcome Message sekarang dinonaktifkan', id)
			} else {
				m2.reply(from, `Membuat BOT menyapa member yang baru join kedalam group chat!\n\nPenggunaan:\n${prefix}welcome on --aktifkan\n${prefix}welcome off --nonaktifkan`, id)
			}
			break
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) m2.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) m2.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
            }
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await m2.reply(from, "Jangan badword bodoh\nDenda +5.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
